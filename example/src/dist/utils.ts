import { NamespaceType, InitialStateContext } from './interface'
...
export const checkIsExit = (namespace: NamespaceType, globalContext: InitialStateContext) => {
    if (globalContext[namespace]) {
        throw new Error("the Context has mounted");
    }
};

export const checkIsNoExit = (namespace: NamespaceType, globalContext: InitialStateContext) => {
    if (!globalContext[namespace]) {
