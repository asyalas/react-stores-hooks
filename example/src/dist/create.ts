...
import { useState, createContext, createElement, useContext, FunctionComponentElement } from "react";
import {
...
const create = (context: InitialStateContext = {}) => {
    ...
    const createStateContext = ({ namespace, initialState }: CreateContextOptions) => {
        checkNameSpace(namespace);
    ...
    const ContextProvider = (props: ContextProviderProps): FunctionComponentElement<any> => {
    ...
    const useDispatchContext = (namespace: NamespaceType): DispatchContextType => {
        checkNameSpace(namespace);
        checkIsNoExit(namespace, globalContext)
        const store: any = globalContext[namespace]
